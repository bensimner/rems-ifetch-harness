...
class Register:
    ...
    def __repr__(self):
        return '{}:{}({})'.format(self.processor, ('X' if self.size == 64 else 'W'), self.register)
...
class RegisterState:
    ...
    def to_ll(self, platform, proc, plabels):
        return ll.RegisterState(register=self.register.to_ll(platform, proc), value=self._value_to_ll(plabels))

    def to_ll_inp(self, platform, proc, outs, plabels):
        return ll.RegisterInputState(register=self.register.to_ll(platform, proc)
...
class State:
    ...
    def to_ll(self, platform, plabels):
        ll_rs = [ll.RegisterState(rs.register.to_ll(platform), rs._value_to_ll(plabels)) for rs in self.registers]
...
class Chunk:
    ...
    def to_ll(self, platform, process, outs, plabels):
        clobbers = [r.to_ll(platform, process.name) for r in self.clobbers(process, outs)]
        in_regs  = [rs.to_ll_inp(platform, process.name, outs, plabels) for rs in self.in_registers(process)]
        out_regs = [r.to_ll(platform, process.name) for r in self.out_registers(process, outs)]
...
class Parser:
        ...
        if v.startswith('"'):
            ...
            if op == 'nop':
                if self.platform == 'aarch64':
                    return 3573751839  # NOP
                elif self.platform == 'ppc':
                    return 0x60000000
            elif op == 'nop2':
                if self.platform == 'aarch64':
                    return 0b11010101000000110010_0000110_11111
                elif self.platform == 'ppc':
                    raise ValueError('Cannot generate NOP2 for PPC')
            elif op == 'nop3':
                if self.platform == 'aarch64':
                    return 0b11010101000000110010_0000111_11111
                elif self.platform == 'ppc':
    ...
    def parse_state(self, state, implicit_process=None):
        ...
        if state:
            ...
            for (reg, value) in pairs:
                reg = reg.strip()
                value = value.strip()
                regs.append(RegisterState(self.parse_reg(reg, implicit_process=implicit_process), self.parse_value(value)))
        return State(regs)
    ...
    def parse(self, s, litmus_name='N/A'):
        ...
        return Litmus(self.parse_mem_state(init), procs, self.parse_state(post['post']), original.lower(), litmus_name, platform, labels, tags)
...
def dumps(litmus, **kws):
    ...
    with open('litmoose_template.jinja2.c') as f:
        ...
        import sys
        return t.render(shuffled=shuffled, litmus=litmus_ll, ll=ll, **builtins.__dict__, **kws)
